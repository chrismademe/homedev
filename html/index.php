...
<html>
    <head>
        ...
        <link rel="stylesheet" href="//fonts.googleapis.com/css?family=Lato:300,400,600,700,900|Source+Code+Pro:400,600">
        ...
        <header class="top">
            <div class="wrapper">
                ...
                foreach ( $devtools as $tool ) {
                    printf( '<a href="%1$s">%2$s</a>', $tool['url'], $tool['name'] );
                }
                ?>
                </div>
            </div>
...
<?php
		    foreach ( $dir as $d ) {
		        ...
		        foreach( glob( $d ) as $file )  {
		            ...
		            if ( ! empty( $adminurl ) )
			            printf( '<a class="%2$s icon" href="%1$s">Admin</a>', $adminurl, is_dir( $file . '/html/wp-admin' ) ? 'wp' : 'admin' );
